<template>
  <div class="skill-balance-display">
    <div size="sm" class="my-2 my-sm-0 mr-3" variant="primary" v-tooltip="'Buy PGS'" @click="onBuySkill">
...
export default Vue.extend({
  computed: {
    ...
    formattedTotalSkillBalance(): string {
      ...
      return `${toBN(skillBalance).toFixed(4)} PGS`;
    },
    ...
    totalSkillTooltipHtml() {
      ...
      let html =  toBN(skillBalance).toFixed(4) + ' PGS';

      if(parseFloat(skillRewards) !== 0){
        html += '<br>+ WITHDRAWABLE ' + toBN(skillRewards).toFixed(4) + ' PGS';
      }

      if(parseFloat(inGameOnlyFundsBalance) !== 0){
        html += '<br>+ IN GAME ONLY ' + toBN(inGameOnlyFundsBalance).toFixed(4) + ' PGS';
      }
