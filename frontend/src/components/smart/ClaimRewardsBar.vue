<template>
  <div class="body main-font">
    <b-navbar>
      ...
      <b-nav-item
        ...
        <span class="gtag-link-others" tagname="claim_skill" v-tooltip.bottom="'Tax is being reduced by 1% per day.' + getTaxTimerNextTick">
          <strong>PGS</strong> {{ formattedSkillReward }}
    ...
    <b-modal class="centered-modal" ref="stake-suggestion-modal" title="Stake PGS"
      @ok="$router.push({ name: 'select-stake-type' })" ok-only ok-title="Go to Stake" >
        You can avoid paying the 15% tax by staking unclaimed PGS rewards for 7 days. If you stake your PGS now, we'll give you a
        50% bonus in-game only PGS that you can use right away!
    ...
    <b-modal class="centered-modal" ref="claim-confirmation-modal" title="Claim PGS" ok-title="I am sure"
      @ok="onClaimTokens()"> You are about to {{ (this.rewardsClaimTaxAsFactorBN > 0)?"pay " + formattedRewardsClaimTax +
      " tax for early withdrawal, costing you " + this.formattedTaxAmount + " PGS. You will also " : "" }}
      miss out on {{formattedBonusLost}} bonus PGS. Are you sure
...
export default Vue.extend({
  ...
  computed: {
    ...
    getTaxTimerNextTick(): string {
      ...
      // if has no PGS rewards do not display timer next tick.
