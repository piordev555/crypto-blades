      ...
      <b-container v-if="fightResults.isVictory">
        <b-row>
          ...
          <b-col class="text-center no-padding">
            <h4>
              {{formattedUsd}}
              <Hint text="PGS earned is based on gas costs of the network plus a factor of your power" />
            </h4>
...
export default Vue.extend({
  ...
  computed: {
    ...
    formattedSkill(): string {
      return `(${toBN(fromWeiEther(this.fightResults.skillGain)).toFixed(6)} PGS)`;
    },
    formattedSkillTooltip(): string {
      return fromWeiEther(this.fightResults.skillGain)+' PGS';
    },
